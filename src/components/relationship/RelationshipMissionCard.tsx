...
import { Mail, Linkedin, MessageCircle, MoreHorizontal, Star, Loader2, Bookmark, Archive, ArchiveRestore, AlarmClock } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-[150px] flex flex-col gap-0.5">
        ...
        {isSnoozed && rel.excludedUntil && (
          <div className="flex items-center gap-1 mt-1">
            <AlarmClock className="w-3 h-3 text-amber-500/70" />
            <span className="text-[10px] text-amber-500/70 font-medium">
              Resurfaces {new Date(rel.excludedUntil).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            </span>
          </div>
        )}
      </div>
